000100 01  W475DGR.                                                             
000200*                                 LÄNK-COPYTEXT FÖR W4053900              
000300     03 IDDISTR              PIC S9(5)           COMP-3.                  
000400*                                 DISTRIKTNUMMER                          
000500     03 IDKUNDNR             PIC S9(7)           COMP-3.                  
000600*                                 KUNDNUMMER                              
000700     03 KDFRAKT              PIC S9(3)           COMP-3.                  
000800*                                 FRAKTSÄTT DC TILL KUND                  
000900     03 IDSKEPPN             PIC S9(7)           COMP-3.                  
001000*                                 SKEPPNINGSNUMMER                        
001100     03 IDDC                 PIC X(2).                                    
001200*                                 IDENTIFIERARE LAGER                     
001300     03 IDPRT                PIC X(3).                                    
001400*                                 LOGISK PRINTERIDENTITET                 
001500     03 IDSYSTEM             PIC X(4).                                    
001600*                                 VOLVO VCCS SYSTEMNUMMER                 
001700     03 IDTRPTNR             PIC S9(3)           COMP-3.                  
001800*                                 TRANSPORTIDENTITET                      
001900     03 DASKEPPN             PIC 9(8).                                    
002000*                                 SKEPPNINGSDATUM  (ÅÅÅÅMMDD)             
002100*** END OF VILMAII-COPY LENGTH= 32 BYTES                                  
