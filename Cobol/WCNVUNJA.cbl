...
000800*                                                                         
000900*    FUNCTION:                                                            
001000*        CONVERSION FROM UNICODE (UCS2 OR UTF8) TO JAPANSESE              
001100*        EBCDIC CODEPAGES (CP290/CS1172 AND CP300/CS1001                  
001200*        = CCSID 930)                                                     
001300                                                                          
001400     SKIP3                                                                
...
001900 77  IDPGM                       PIC X(8)    VALUE 'WCNVUNJA'.            
...
005841     EJECT                                                                
005850*    -- WORK FIELDS WHEN CONVERTING VALUE TO HEX-CHAR                     
005860 01  TEMP-4BYTE-X.                                                        
005870     03 TEMP-4BYTE-FIRST2        PIC XX      VALUE LOW-VALUE.             
005880     03 TEMP-2BYTE-X.                                                     
005890         05 TEMP-2BYTE-N         PIC S9(4)   COMP.                        
005891 01  TEMP-4BYTE-N REDEFINES TEMP-4BYTE-X                                  
005892                                 PIC S9(9)   COMP.                        
005893                                                                          
005894*    -- TABLE USED WHEN CONVERTING A NUMERIC VALUE TO HEX                 
005895 01  HEX-CHAR                    PIC X(16)  VALUE                         
005896                                 '0123456789ABCDEF'.                      
005897 77  IXH                         PIC S9(4)   COMP.                        
005898 77  IXU                         PIC S9(4)   COMP.                        
005899                                                                          
005900*    -- WORK FIELDS FOR CREATING HEX-CHAR VALUES                          
...
006100*01  -COPY WCNVU290                                                       
006200     EJECT                                                                
006300*01  -COPY WCNVU300                                                       
...
007700                                                                          
007800*      -- TRY CONVERTING TO SINGLEBYTE CODE (CP290)                       
007900       MOVE TECONV-TEMP (IXF + BO:1)     TO WBYTE                         
008000       ADD  WNUM  1                  GIVING IX0                           
008100       MOVE TAB2-IND-U290 (IX0)          TO IX1                           
008200       MOVE TECONV-TEMP (IXF + 1 - BO:1) TO WBYTE                         
008300       ADD  WNUM  1                  GIVING IX2                           
008400       MOVE UCS-TO-CP290 (IX1, IX2)     TO TEMP-1BYTE                     
008500       IF TEMP-1BYTE = X'3F'                                              
008600*        -- TRY CONVERTING TO DOUBLEBYTE CODE (CP300)                     
008700         MOVE TECONV-TEMP (IXF + BO:1)     TO WBYTE                       
008800         ADD  WNUM  1                  GIVING IX0                         
008900         MOVE TAB2-IND-U300 (IX0)          TO IX1                         
009000         MOVE TECONV-TEMP (IXF + 1 - BO:1) TO WBYTE                       
009100         ADD  WNUM  1                  GIVING IX2                         
009200         MOVE UCS-TO-CP300-X (IX1, IX2)    TO TEMP-2BYTE                  
009300         IF TEMP-2BYTE = X'FEFE'                                          
009320         AND TECONV-TEMP (IXF:2) NOT = X'FFFD'                            
009330*        -- FFFD ÄR DUBBELBYTE "SUB" OCH SKA BLI FEFE                     
009340*        -- DENNA ÄR ALLTSÅ OK MEDAN ANDRA SKA GE FELMED                  
009400*        -- TEXT COULD NOT BE CONVERTED                                   
009500           MOVE 'F' TO KDSVAR                                             
009510           MOVE IXF TO IXF-DISP                                           
009511           PERFORM S01-HEX-TO-CHAR                                        
009520           STRING 'POS ' IXF-DISP ' ' TEMP-HEX-ENT '. '                   
009522                DELIMITED BY SIZE                                         
009530           INTO BEFEL WITH POINTER EPTR                                   
009600         END-IF                                                           
009700       END-IF                                                             
...
012650                                                                          
012660     IF FLUTF8 = JA OR YES                                                
012670*      -- UTF8 - COMPUTE LENGTH OF TEXT IN FROM FIELD                     
012680*      -- (IGNORE TRAILING NULL VALUES)                                   
012690       MOVE ZERO TO UTF8-LENG                                             
012691       INSPECT FUNCTION REVERSE (TECONV-FROM)                             
012692         TALLYING UTF8-LENG FOR LEADING LOW-VALUE                         
012693       IF UTF8-LENG = ZERO                                                
012694*      -- TRY TRAILING SPACE INSTEAD                                      
012695         INSPECT FUNCTION REVERSE (TECONV-FROM)                           
012696           TALLYING UTF8-LENG FOR LEADING SPACE                           
012697       END-IF                                                             
012698       COMPUTE UTF8-LENG = TECONV-LENG - UTF8-LENG                        
012699*      -- CONVERT UTF8 INPUT TO UCS2 BEFORE CONVERTING TO EBCDIC          
012700       MOVE LENGTH OF TECONV-TEMP TO UCS2-LENG                            
012701       CALL WCNVUTFU USING TECONV-FROM UTF8-LENG                          
012702                           TECONV-TEMP UCS2-LENG                          
012703       MOVE UCS2-LENG TO FLENG                                            
012704     ELSE                                                                 
012705*      -- UCS2 - COMPUTE LENGTH OF TEXT IN FROM FIELD                     
012706*      -- (IGNORE TRAILING DOUBLE-BYTE SPACES)                            
012707       MOVE ZERO TO FLENG                                                 
012708       INSPECT FUNCTION REVERSE (TECONV-FROM)                             
012709         TALLYING FLENG FOR LEADING SPACE                                 
012710       COMPUTE FLENG = TECONV-LENG - FLENG                                
012711       MOVE TECONV-FROM          TO TECONV-TEMP                           
012712     END-IF                                                               
012720     .                                                                    
012800     EJECT                                                                
...
018900     .                                                                    
018910     EJECT                                                                
018920 S01-HEX-TO-CHAR  SECTION.                                                
018930     SKIP2                                                                
018940     MOVE TECONV-TEMP (IXF + BO:1)     TO TEMP-2BYTE-X (1:1)              
018950     MOVE TECONV-TEMP (IXF + 1 - BO:1) TO TEMP-2BYTE-X (2:1)              
019000     MOVE 4 TO IXU                                                        
019100     MOVE LOW-VALUE TO TEMP-4BYTE-FIRST2                                  
019200     PERFORM 4 TIMES                                                      
019300       DIVIDE TEMP-4BYTE-N BY 16 GIVING TEMP-4BYTE-N                      
019400              REMAINDER IXH                                               
019500       MOVE HEX-CHAR (IXH + 1:1) TO TEMP-HEXNR (IXU:1)                    
019600       SUBTRACT 1 FROM IXU                                                
019700     END-PERFORM                                                          
019810     .                                                                    
