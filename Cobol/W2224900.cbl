000100 ID DIVISION.                                                             
000200 PROGRAM-ID.     W2224900.                                                
000300 AUTHOR.         STENING INGER.                                           
000400 DATE-WRITTEN.   15/12/29.                                                
000500 DATE-COMPILED.                                                           
000600                                                                          
000700                                                                          
000800*    FUNCTION:                                                            
000900*        UPDATE ASSETS IN CDC                                             
001000*                                                                         
001100*        THE PROGRAM UPDATES   WDK6                                       
001200*                                                                         
001300                                                                          
001400     SKIP3                                                                
001500 ENVIRONMENT DIVISION.                                                    
001600     SKIP2                                                                
001700 INPUT-OUTPUT SECTION.                                                    
001800                                                                          
001900 FILE-CONTROL.                                                            
002000     SKIP2                                                                
002100*          --- INPUT FILE FROM W22239                                     
002200     SELECT W22239                     ASSIGN TO W22249D1.                
002300     EJECT                                                                
002400 DATA DIVISION.                                                           
002500     SKIP3                                                                
002600 FILE SECTION.                                                            
002700     SKIP3                                                                
002800 FD  W22239                                                               
002900     RECORDING       F                                                    
003000     BLOCK CONTAINS  0.                                                   
003100                                                                          
003200*01  -COPY W22239      -L.                                                
003300     EJECT                                                                
003400 WORKING-STORAGE SECTION.                                                 
003500                                                                          
003600 77  IDPGM                       PIC X(8)    VALUE 'W2224900'.            
003700 01  CHKP-VAR.                                                            
003800     03 CHKP-MSG-IO-AREA-LENGTH  PIC S9(9)   VALUE +32 COMP SYNC.         
003900     03 CHKP-MSG-IO-AREA         PIC X(32)   VALUE SPACE.                 
004000     03 CHKP-AREA-LENGTH         PIC S9(9)   VALUE +32 COMP SYNC.         
004100     03 CHKP-AREA                PIC X(32)   VALUE SPACE.                 
004200     03 CHKP-ANT                 PIC S9(3)   VALUE +0   COMP-3.           
004300     03 CHKP-MAX                 PIC S9(3)   VALUE +500 COMP-3.           
004400 77  YES                         PIC X       VALUE 'J'.                   
004500 77  NOO                         PIC X       VALUE 'N'.                   
004600     SKIP2                                                                
004700 01  ERROR-TEXT.                                                          
004800     03  FILLER                  PIC X(8)    VALUE 'ERR-TEXT'.            
004900     03  ERROR-TEXT-STR          PIC X(72)   VALUE SPACE.                 
005000                                                                          
005100 77  W22239-EOF-SW               PIC X       VALUE 'N'.                   
005200     88  END-OF-W22239                       VALUE 'Y'.                   
005300     EJECT                                                                
005400 01  TODAYS-DATE                 PIC 9(6)    VALUE ZERO.                  
005500 01  FILLER REDEFINES TODAYS-DATE.                                        
005600     03  TODAYS-DATE-YEAR        PIC 9(2).                                
005700     03  TODAYS-DATE-MONTH       PIC 9(2).                                
005800     03  TODAYS-DATE-DAY         PIC 9(2).                                
005900     EJECT                                                                
006000 01  GENERAL-SUBPROGRAMS.                                                 
006100*                                                                         
006200     03  CBLTDLI                 PIC X(8)    VALUE 'CBLTDLI '.            
006300     03  FELLOG                  PIC X(8)    VALUE 'FELLOG  '.            
006400     03  POSTSUM                 PIC X(8)    VALUE 'POSTSUM'.             
006500     EJECT                                                                
006600*    --- PARAMETRAR TILL POSTSUM                                          
006700*                                                                         
006800*01  -COPY W0005   -PRE  POSTSUM-                                         
006900     EJECT                                                                
007000 01  IN-AREA-START               PIC X(24)   VALUE                        
007100                                             'IN-AREA-START'.             
007200*01  AREA -COPY W22239     -PRE IN-                                       
007300*                                                                         
007400     EJECT                                                                
007500 01  FILLER                      PIC X(16)   VALUE 'IMS-WS'.              
007600     SKIP3                                                                
007700 01  KEYS-TILL-DLI.                                                       
007800     03  W-IDARTNR-X.                                                     
007900         05  W-IDARTNR           PIC S9(9)   VALUE ZERO COMP-3.           
008000     03  W-KDSEGKEY-X.                                                    
008100         05  W-KDSEGKEY          PIC X(1)    VALUE '1'.                   
008200     SKIP2                                                                
008300*    --- STATUS-KOD FRÅN IMS                                              
008400 01  STATUS-WS                   PIC XX.                                  
008500     88  SEGMENT-FOUND                       VALUE '  '.                  
008600     88  SEGMENT-FOUND-EXISTS                VALUE 'II'.                  
008700     88  SEGMENT-MISSING                     VALUE 'GE'.                  
008800     88  SEGMENT-NOMORE                      VALUE 'GB'.                  
008900     88  IMS-NOT-OK                          VALUE 'XD'.                  
009000     SKIP2                                                                
009100 01  GOOD-STATUSCODES.                                                    
009200     03  GOOD-STATUS OCCURS 5 INDEXED BY STATUS-IX PIC XX.                
009300     SKIP3                                                                
009400 01  SSA1                        PIC X(64).                               
009500 01  SSA2                        PIC X(64).                               
009600     EJECT                                                                
009700*    --- IMS FUNCTION CODES                                               
009800*01  -COPY W0003                                                          
009900     EJECT                                                                
010000*    ---  DLI INPUT-OUTPUT AREA                                           
010100                                                                          
010200 01  FILLER                    PIC X(16) VALUE 'DLI-IO-WDK611 '.          
010300 01  DLI-IO-WDK611.                                                       
010400*    03  -COPY WDK611                                                     
010500     EJECT                                                                
010600 LINKAGE SECTION.                                                         
010700                                                                          
010800*01  -COPY W0009   -PRE MSG-                                              
010900                                                                          
011000*01  -COPY W0008  -PRE WDK6-                                              
011100     05  FILLER                  PIC X.                                   
011200     EJECT                                                                
011300 PROCEDURE DIVISION  USING MSG-PCB WDK6-PCB.                              
011400 MAIN SECTION.                                                            
011500     ENTRY 'DLITCBL' USING MSG-PCB WDK6-PCB.                              
011600                                                                          
011800     PERFORM A-INIT                                                       
011810                                                                          
011900     PERFORM S01-READ-W22239                                              
012000     PERFORM UNTIL END-OF-W22239                                          
012100       MOVE IN-IDARTNR            TO W-IDARTNR                            
012200       PERFORM IMS-GHU-WDK611                                             
012300       IF SEGMENT-FOUND                                                   
012400          MOVE IN-KVPB-TPO     TO CLAG-KVPB-TPO                           
012500          MOVE IN-FLMANPB      TO CLAG-FLMANPB                            
012800          PERFORM IMS-REPL-WDK611                                         
012900          IF CHKP-ANT > CHKP-MAX                                          
013000            PERFORM X-TAKE-CHECKPOINT                                     
013100          END-IF                                                          
013200       END-IF                                                             
013300                                                                          
013400       PERFORM S01-READ-W22239                                            
013500     END-PERFORM                                                          
013600                                                                          
013700                                                                          
013800     PERFORM Z-FINIT                                                      
013900                                                                          
014000     MOVE ZERO TO RETURN-CODE                                             
014100     GOBACK                                                               
014200     .                                                                    
014300     EJECT                                                                
014400 A-INIT SECTION.                                                          
014600                                                                          
014700     PERFORM IMS-RESTART                                                  
014800                                                                          
014900     OPEN INPUT W22239                                                    
015000                                                                          
015100                                                                          
015200     MOVE IDPGM TO POSTSUM-PROGNAMN                                       
015300     .                                                                    
015400     EJECT                                                                
015500 Z-FINIT SECTION.                                                         
015700                                                                          
015800     CLOSE W22239                                                         
015900     SKIP2                                                                
016000     MOVE 'S' TO POSTSUM-OPKOD                                            
016100     CALL POSTSUM USING POSTSUM-PARM                                      
016200     .                                                                    
016300     EJECT                                                                
016400 S01-READ-W22239   SECTION.                                               
016500     SKIP2                                                                
016600     READ W22239 INTO IN-AREA                                             
016700     AT END                                                               
016800        SET END-OF-W22239 TO TRUE                                         
016900                                                                          
017000     NOT AT END                                                           
017100        MOVE 'W22239'   TO POSTSUM-FDNAMN                                 
017200        MOVE 'W22239D1' TO POSTSUM-DDNAMN2                                
017300        MOVE SPACE      TO POSTSUM-TRANSTYP                               
017400        CALL POSTSUM USING POSTSUM-PARM                                   
017500                                                                          
017600     END-READ                                                             
017700     .                                                                    
017800     EJECT                                                                
017900 X-TAKE-CHECKPOINT   SECTION.                                             
018000                                                                          
018100* --- AT CHECKPOINT YOU LOSE GN-POSITION IN THE BASE                      
018200* --- SAVE DATABASE KEYS IF NECESSARY                                     
018300     PERFORM IMS-CHECKPOINT                                               
018400     MOVE ZERO TO CHKP-ANT                                                
018500* --- REREAD DATABASE IF NECESSARY                                        
018600     .                                                                    
018700     EJECT                                                                
018800* --- IMS SECTIONS  ---                                                   
018900                                                                          
019000 IMS-GHU-WDK611 SECTION.                                                  
019100                                                                          
019200     STRING 'WDK601  (IDARTNR  =' W-IDARTNR-X  ')'                        
019300          DELIMITED BY SIZE INTO SSA1                                     
019400     STRING 'WDK611  (KDSEGKEY =' W-KDSEGKEY-X ')'                        
019500          DELIMITED BY SIZE INTO SSA2                                     
019600     MOVE '  GE'              TO GOOD-STATUSCODES                         
019700     CALL CBLTDLI USING GHU WDK6-PCB DLI-IO-WDK611 SSA1 SSA2              
019800     MOVE WDK6-STATUS-CODE    TO STATUS-WS                                
019900     PERFORM IMS-STATUSCHECK                                              
020000     .                                                                    
020100                                                                          
020200 IMS-REPL-WDK611 SECTION.                                                 
020300                                                                          
020400     MOVE '  '             TO GOOD-STATUSCODES                            
020500     CALL CBLTDLI USING REPL WDK6-PCB DLI-IO-WDK611                       
020600     MOVE WDK6-STATUS-CODE TO STATUS-WS                                   
020700     PERFORM IMS-STATUSCHECK                                              
020800     ADD +1                TO CHKP-ANT                                    
020900     .                                                                    
021000     EJECT                                                                
021100 IMS-RESTART SECTION.                                                     
021200     SKIP2                                                                
021300     MOVE SPACE TO CHKP-MSG-IO-AREA                                       
021400     MOVE '  ' TO GOOD-STATUSCODES                                        
021500     CALL CBLTDLI USING XRST MSG-PCB                                      
021600                        CHKP-MSG-IO-AREA-LENGTH CHKP-MSG-IO-AREA          
021700                        CHKP-AREA-LENGTH CHKP-AREA                        
021800     MOVE MSG-STATUS-CODE TO STATUS-WS                                    
021900     PERFORM IMS-STATUSCHECK                                              
022000     .                                                                    
022100     SKIP3                                                                
022200 IMS-CHECKPOINT SECTION.                                                  
022300     SKIP2                                                                
022400     MOVE SPACE TO CHKP-MSG-IO-AREA                                       
022500     MOVE '  XD' TO GOOD-STATUSCODES                                      
022600     CALL CBLTDLI USING CHKP MSG-PCB                                      
022700                        CHKP-MSG-IO-AREA-LENGTH CHKP-MSG-IO-AREA          
022800                        CHKP-AREA-LENGTH CHKP-AREA                        
022900     MOVE MSG-STATUS-CODE TO STATUS-WS                                    
023000     PERFORM IMS-STATUSCHECK                                              
023100                                                                          
023200     IF IMS-NOT-OK                                                        
023300       MOVE 'IMS CONTROL REGION IS NOT ACCESSIBLE'                        
023400                                       TO ERROR-TEXT-STR                  
023500       DISPLAY ERROR-TEXT                                                 
023600       CALL FELLOG                                                        
023700     END-IF                                                               
023800     .                                                                    
023900     EJECT                                                                
024000 IMS-STATUSCHECK SECTION.                                                 
024100     SKIP2                                                                
024200     SET STATUS-IX TO 1                                                   
024300     SEARCH GOOD-STATUS                                                   
024400       AT END                                                             
024500         STRING ' FELAKTIG STATUSKOD FRÅN IMS: ' STATUS-WS                
024600           DELIMITED BY SIZE INTO ERROR-TEXT                              
024700         DISPLAY ERROR-TEXT                                               
024800         CALL FELLOG                                                      
024900       WHEN GOOD-STATUS (STATUS-IX) = STATUS-WS                           
025000         CONTINUE                                                         
025100     END-SEARCH                                                           
025200     .                                                                    
